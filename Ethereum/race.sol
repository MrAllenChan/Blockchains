...
contract Dao {
    ...
    function deposit() payable public {
        ...
        /** update total balance and individual balance */
        ...
        if (!curProposal.isSealed && curProposal.didVote[msg.sender]) {
            if (curProposal.voteRecord[msg.sender]) { /** vote Yes */
                curProposal.totalYes += tokensGained;
            } else if (!curProposal.voteRecord[msg.sender]) { /** vote No */
    ...
    function withdraw(uint256 tokensToWithdraw) public returns (bool _bool) {
        require (balances[msg.sender] >= tokensToWithdraw && totalBalance >= tokensToWithdraw);
        ...
        (bool result,) = msg.sender.call.value(tokensToWithdraw * valuation / 10)("");
        if (!result) {
            revert();
        }
